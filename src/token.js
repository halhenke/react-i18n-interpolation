 ...
 *
 * May indicate an `error` but must return reasonable properties regardless. This implementation is
 * opinionated in that non-primitive substitutions must be presented in a {[key]:value} hash.
 *
...
export const defaultToToken = (candidate, i) => {
  ...
  const error = !!candidate && ['object', 'function', 'symbol'].includes(typeof candidate) &&
    !key && 'All non-primitive substitutions must be "keyed"';
